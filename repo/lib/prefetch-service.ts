 ...
 * This allows the code to work in Edge Runtime where tfjs may not be available
 */
async function loadTensorFlow(): Promise<boolean> {
  ...
  tfLoadPromise = (async (): Promise<boolean> => {
    try {
      // Try to load @tensorflow/tfjs using dynamic import
      ...
      console.warn('[AI-MODEL] ⚠️ Could not load @tensorflow/tfjs, using rule-based fallback')
  ...
  modelLoadPromise = (async () => {
    try {
      ...
      try {
        ...
        // Edge Runtime may not support TensorFlow.js, gracefully fall back
